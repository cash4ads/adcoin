...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Adcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Adcoin Signed Message:\n";
...
{
    // Adcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    if (IsCoinBase())
    {
        if (vin[0].scriptSig.size() < 2 || vin[0].scriptSig.size() > 1024)
...
{
    // Adcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x44ee704868b792668c9964b95ded63c7b15588b3911cec8e8632daf8d6d8fba1 ");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "You like cash4ads? Their early work was a little too new wave for my taste. But when Adcoin came out in '14, I think they really came into their own, commercially and artistically. The whole currency has a clear, crisp look, and a new sheen of consummate professionalism that really gives the coins a big boost. They've been compared to Dogecoin, but I think Adcoin has a far more bitter, cynical sense of humor.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1982 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = 1391645091;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 0;

        if (fTestNet)
        {
            block.nTime    = 1391645094;
            block.nNonce   = 1310912;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7e65aa4477fb622e66530cfcc72299400eabf847f885727616ba2b996ff9f87d"));
        block.print();
        ...
        if(block.GetHash() != hashGenesisBlock)
        {
            ...
            while(true)
            {
                scrypt_hash_256(BEGIN(block.nVersion), BEGIN(thash));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Adcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AdcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AdcoinMiner(CWallet *pwallet)
{
    printf("AdcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running AdcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            loop
            {
                scrypt_hash_256(BEGIN(pblock->nVersion), BEGIN(thash));
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AdcoinMiner, pwallet));
}
